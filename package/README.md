...
- [CI/CD integrations](https://testdino.com/features/cicd-integrations): GitHub Actions, GitLab CI, Azure DevOps, Jenkins, CircleCI, TeamCity, AWS CodeBuild, and Bitbucket Pipelines
...
Delivery degrades gracefully: if the streaming connection is unavailable, events fall back to an alternate transport automatically. Reporter errors never fail your tests. A TestDino outage affects reporting only, and your suite exits on Playwright's own result.
...
Tests run normally. Only event reporting is affected.
