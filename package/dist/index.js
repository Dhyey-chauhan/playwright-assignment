...
var PERMANENT_HTTP_STATUSES = /* @__PURE__ */ new Set([400, 401, 403, 404, 405, 413]);
...
var HttpClient = class {
  ...
  async resolveRunLink(runId) {
    try {
      ...
      if (import_axios.default.isAxiosError(error) && error.response?.status === 404) return null;
      if (import_axios.default.isAxiosError(error) && error.response) {
        throw new HttpDeliveryError(error.response.status, this.getErrorMessage(error), { cause: error });
      }
      throw error;
  ...
  getErrorMessage(error) {
    if (import_axios.default.isAxiosError(error)) return error.response?.data?.message || error.message;
...
var EventDeliveryManager = class {
  ...
  async tryHttpWithRetry(events) {
    ...
    for (let attempt = 1; attempt <= this.maxAttempts; attempt++) {
      try {
        await this.httpClient.sendEvents(fallbackUrl, events);
        const first = events[0];
        const last = events[events.length - 1];
        const seqRange = "sequence" in first ? `(seq ${first.sequence}-${last.sequence})` : "";
        this.logger.debug(`Flushed ${events.length} events via http ${seqRange}`.trim());
        this.lastDeliveryAt = Date.now();
        ...
        if (error instanceof HttpDeliveryError && isPermanentHttpStatus(error.status)) {
          this.logger.error(`HTTP fallback permanent error (${error.status}) \u2014 events discarded`);
          throw error;
        }
        ...
        if (attempt < this.maxAttempts) {
          const delay = this.getBackoffDelay(attempt);
          this.logger.debug(`HTTP attempt ${attempt}/${this.maxAttempts} failed: ${message} (retrying in ${delay}ms)`);
          await this.sleep(delay);
        } else {
          this.logger.error(`HTTP fallback delivery failed after ${this.maxAttempts} attempts: ${message}`);
        }
      }
    }
    throw lastError ?? new Error("HTTP fallback delivery failed \u2014 no attempts made (maxAttempts must be >= 1)");
  }
...
var GitMetadataCollector = class extends BaseMetadataCollector {
    ...
    if (env.CODEBUILD_WEBHOOK_TRIGGER) {
      ...
      if (prMatch?.[1]) {
        ...
        if (env.CODEBUILD_SOURCE_REPO_URL) {
          info.prUrl = this.buildCodeBuildPRUrl(env.CODEBUILD_SOURCE_REPO_URL, prMatch[1]);
        }
      }
    }
  }
  /**
   * Build the PR URL for the CodeBuild source repo, parsing the hostname so
   ...
   * link.
   */
  buildCodeBuildPRUrl(repoUrl, prId) {
  ...
  async readGitHubEventFile() {
    ...
    if (!(0, import_path2.isAbsolute)((0, import_path2.normalize)(eventPath))) {
...
var SystemMetadataCollector = class extends BaseMetadataCollector {
  ...
  getMemoryInfo() {
    try {
      return `${((0, import_os2.totalmem)() / (1024 * 1024 * 1024)).toFixed(1)} GB`;
    } catch {
      return "unknown";
    }
  }
...
function isAllowedExtension(fileName, allowed = DEFAULT_ALLOWED_FILE_TYPES) {
  const ext = (0, import_path3.extname)(fileName).slice(1).toLowerCase();
...
var ArtifactUploadClient = class {
  ...
  async doPut(uploadUrl, body, contentType) {
    await import_axios2.default.put(uploadUrl, body, {
      headers: {
...
var CoverageMerger = class {
  ...
  constructor(options) {
    if (options?.include && options.include.length > 0) {
      this.isIncluded = (0, import_picomatch.default)(options.include);
    }
    if (options?.exclude && options.exclude.length > 0) {
      this.isExcluded = (0, import_picomatch.default)(options.exclude);
    }
...
var colors = {
  success: (text) => supportsColor() ? import_chalk.default.green(text) : text,
  error: (text) => supportsColor() ? import_chalk.default.red(text) : text,
  warning: (text) => supportsColor() ? import_chalk.default.yellow(text) : text,
  info: (text) => supportsColor() ? import_chalk.default.cyan(text) : text,
  bold: (text) => supportsColor() ? import_chalk.default.bold(text) : text,
  dim: (text) => supportsColor() ? import_chalk.default.dim(text) : text,
  white: (text) => supportsColor() ? import_chalk.default.white(text) : text,
  gray: (text) => supportsColor() ? import_chalk.default.gray(text) : text,
  blue: (text) => supportsColor() ? import_chalk.default.blue(text) : text,
  url: (text) => supportsColor() ? import_chalk.default.blue.underline(text) : text
};
var symbols = {
  success: "\u2713",
  // ✓
...
function getModuleDir() {
  const metaUrl = typeof import_meta === "object" ? import_meta?.url : void 0;
  if (metaUrl) return (0, import_path4.dirname)((0, import_url.fileURLToPath)(metaUrl));
  return __dirname;
}
function getPackageVersion() {
  ...
  for (const candidate of candidates) {
    try {
      const raw = (0, import_fs.readFileSync)((0, import_path4.join)(getModuleDir(), candidate), "utf-8");
...
async function fetchLatestVersion() {
  try {
    const response = await import_axios3.default.get(REGISTRY_URL, {
      timeout: CHECK_TIMEOUT_MS,
      headers: { Accept: "application/vnd.npm.install-v1+json" }
    });
    const version2 = response.data?.version;
...
var TestdinoReporter = class _TestdinoReporter {
  ...
  async performAsyncInit(config, suite, token) {
    ...
    try {
      ...
      if (this.deliveryManager) {
        ...
        const startLink = this.shardScopedUrl(this.resolvedRunUrl);
        if (startLink) this.log.runLink(startLink);
  ...
  async onEnd(result) {
    ...
    const summaryData = {
      ...
      runUrl: this.shardScopedUrl(this.resolvedRunUrl ?? await this.runUrlPromise ?? void 0)
    };
    this.log.printRunSummary(result, summaryData);
    if (this.coverageThresholdFailures.length > 0) {
  ...
  shardScopedUrl(url) {
    ...
    return this.shardInfo ? `${url}?shard=${this.shardInfo.current}` : url;
  }
      ...
      if (this.pendingTestEndPromises.size > 0) {
        try {
          await Promise.race([
            ...
            this.timeoutPromise(1e3, "Pending events timeout")
          ]);
        } catch {
        }
      }
    ...
    const sendAndExit = async () => {
      try {
        ...
        const summaryData = {
          ...
          coverageThresholdFailures: this.coverageThresholdFailures,
          artifactsDisabled: !this.artifactsEnabled,
          runUrl: this.shardScopedUrl(this.resolvedRunUrl)
        };
  ...
  async statAttachments(files) {
    const results = await Promise.allSettled(files.map((f) => (0, import_promises4.stat)(f.path)));
  ...
  filterUploadable(stated) {
    ...
    for (const item of stated) {
      ...
      if (isAllowedExtension((0, import_path5.basename)(item.path)) && isWithinSizeLimit(item.sizeBytes)) {
        uploadable.push(item);
      } else {
        const reason = isAllowedExtension((0, import_path5.basename)(item.path)) ? `${item.sizeBytes} bytes exceeds the size limit` : `extension not in the allowlist`;
        this.log.debug(`Attachment '${(0, import_path5.basename)(item.path)}' skipped: ${reason}`);
        preFiltered.push({ name: item.name, contentType: item.contentType, unavailable: true });
      }
    }
  ...
  async putBatch(uploadable, testId, client) {
    ...
    try {
      const request = ready.map((f) => ({
        ...
        name: (0, import_path5.basename)(f.path),
        testId,
  ...
  async snapshotAttachments(uploadable) {
    const reads = await Promise.allSettled(uploadable.map((f) => (0, import_promises4.readFile)(f.path)));
    const ready = [];
    const readFailed = [];
  ...
  maybeCompressCoverageEvent(event) {
    ...
    try {
      const compressed = (0, import_zlib.gzipSync)(Buffer.from(json, "utf8"));
